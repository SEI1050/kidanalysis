...
出力
----
- corrected_iq_tracks_spatial_map.pdf   : x-z実位置に合わせた大判1ページ PDF
- corrected_iq_tracks_spatial_map.png   : 上記の高解像度 PNG
- corrected_iq_tracks_all_folders.pdf   : 従来の複数ページ PDF（9 枚/ページ）
- corrected_iq_tracks_page_01.png など : 従来の各ページ PNG
...
# x-z 実位置対応の大判1ページ図
# -----------------------------------------------------------------------------
SAVE_SPATIAL_MAP = True
SPATIAL_FIGSIZE = (24, 18)
SPATIAL_DPI = 240

# "per_folder"   : すべての小図を個別 zoom
# "per_position" : 同じ (x, z) 座標の再測定どうしで共通軸
# "global"       : 全小図で共通軸
SPATIAL_AXIS_MODE = "per_position"

# True: z が大きい位置を上側に配置。装置上の向きが逆なら False。
SPATIAL_Z_INCREASES_UPWARD = True

# 同一座標に複数フォルダがある場合、その座標枠内で最大3列に並べる。
SPATIAL_MAX_COLUMNS_PER_POSITION = 3

# 隣接座標間隔に対して、小図群が占める幅・高さの割合。
SPATIAL_CELL_WIDTH_FRACTION = 1.95
SPATIAL_CELL_HEIGHT_FRACTION = 1.95
SPATIAL_INNER_GAP_FRACTION = 0.04
...
def extract_folder_position(name: str) -> tuple[float, float, str]:
    """フォルダ名から (x_mm, z_mm, tag) を取り出す。"""
    m = _FOLDER_RE.match(name)
    if not m:
        raise ValueError(
            f"Could not parse x/z position from folder name: {name!r}"
        )
    return float(m.group("x")), float(m.group("z")), (m.group("tag") or "")


def spatial_run_title(name: str, run_index: int, n_runs: int) -> str:
    """実位置図の小図タイトル。座標自体は背景軸に任せ、測定タグだけ表示する。"""
    _, _, tag = extract_folder_position(name)
    if tag:
...
def plot_folder_track(
    ...
    shown_title = result.title if title_text is None else title_text
    ax.set_title(shown_title, fontsize=7 if compact else 10, pad=2 if compact else 4)
    ax.set_xlim(xlim)
    ax.set_ylim(ylim)
    ax.set_aspect("equal", adjustable="box")
    ax.grid(alpha=0.25)
    ax.tick_params(
        labelsize=5.5 if compact else 8,
        length=2 if compact else 3.5,
...
def _minimum_positive_gap(values: list[float], fallback: float = 1.0) -> float:
    unique = np.unique(np.asarray(values, dtype=float))
    if unique.size < 2:
        return fallback
    diffs = np.diff(np.sort(unique))
    positive = diffs[diffs > 0]
    return float(np.min(positive)) if positive.size else fallback
...
def save_spatial_map(
    ...
    """
    横軸を実際の x [mm]、縦軸を実際の z [mm] とし、
    各フォルダの IQ 図を対応座標の位置へ配置して1枚に保存する。

    同じ (x, z) に複数の測定がある場合は、その座標セルの中に分割配置する。
    """
    grouped: dict[tuple[float, float], list[FolderResult]] = {}
    skipped: list[str] = []

    for result in results:
        try:
            x_mm, z_mm, _ = extract_folder_position(result.folder_name)
        except ValueError:
            skipped.append(result.folder_name)
            continue
        grouped.setdefault((x_mm, z_mm), []).append(result)

    if skipped:
        print("[spatial-map] skipped unparsable folders:")
        for name in skipped:
            print(f"  - {name}")

    if not grouped:
        raise RuntimeError("No folders with parseable x/z coordinates for spatial map")

    for group in grouped.values():
        group.sort(key=lambda r: parse_folder_info(r.folder_name))

    x_values = sorted({position[0] for position in grouped})
    z_values = sorted({position[1] for position in grouped})

    dx = _minimum_positive_gap(x_values, fallback=1.0)
    dz = _minimum_positive_gap(z_values, fallback=1.0)

    # 端の座標に置いた小図が背景軸の外へはみ出さないための余白。
    x_pad = 0.72 * dx
    z_pad = 0.72 * dz
    x_data_lim = (min(x_values) - x_pad, max(x_values) + x_pad)
    z_data_lim = (min(z_values) - z_pad, max(z_values) + z_pad)

    cell_width_data = SPATIAL_CELL_WIDTH_FRACTION * dx
    cell_height_data = SPATIAL_CELL_HEIGHT_FRACTION * dz

    fig = plt.figure(figsize=SPATIAL_FIGSIZE)
    map_ax = fig.add_axes([0.075, 0.085, 0.89, 0.82])
    map_ax.set_xlim(x_data_lim)
    map_ax.set_ylim(z_data_lim)
    if not SPATIAL_Z_INCREASES_UPWARD:
        map_ax.invert_yaxis()

    map_ax.set_xticks(x_values)
    map_ax.set_yticks(z_values)
    map_ax.set_xlabel("Physical x position [mm]", fontsize=15, labelpad=12)
    map_ax.set_ylabel("Physical z position [mm]", fontsize=15, labelpad=12)
    map_ax.tick_params(labelsize=12)
    map_ax.grid(True, linestyle="--", linewidth=0.9, alpha=0.35)
    map_ax.set_axisbelow(True)

    map_bbox = map_ax.get_position()
    x_span = x_data_lim[1] - x_data_lim[0]
    z_span = z_data_lim[1] - z_data_lim[0]
    cell_width_fig = map_bbox.width * cell_width_data / x_span
    cell_height_fig = map_bbox.height * cell_height_data / z_span

    def data_to_figure(x_mm: float, z_mm: float) -> tuple[float, float]:
        fx = map_bbox.x0 + (x_mm - x_data_lim[0]) / x_span * map_bbox.width
        if SPATIAL_Z_INCREASES_UPWARD:
            fy = map_bbox.y0 + (z_mm - z_data_lim[0]) / z_span * map_bbox.height
        else:
            fy = map_bbox.y0 + (z_data_lim[1] - z_mm) / z_span * map_bbox.height
        return fx, fy

    for (x_mm, z_mm), position_results in sorted(
        grouped.items(), key=lambda item: (item[0][1], item[0][0])
    ):
        n_runs = len(position_results)
        ncols = min(SPATIAL_MAX_COLUMNS_PER_POSITION, n_runs)
        nrows = math.ceil(n_runs / ncols)

        gap_x = SPATIAL_INNER_GAP_FRACTION * cell_width_fig
        gap_y = SPATIAL_INNER_GAP_FRACTION * cell_height_fig
        sub_width = (cell_width_fig - (ncols - 1) * gap_x) / ncols
        sub_height = (cell_height_fig - (nrows - 1) * gap_y) / nrows

        center_x, center_y = data_to_figure(x_mm, z_mm)
        cell_left = center_x - 0.5 * cell_width_fig
        cell_bottom = center_y - 0.5 * cell_height_fig

        if SPATIAL_AXIS_MODE == "global":
            shared_xlim, shared_ylim = global_xlim, global_ylim
        elif SPATIAL_AXIS_MODE == "per_position":
            shared_xlim, shared_ylim = robust_limits(position_results)
        elif SPATIAL_AXIS_MODE == "per_folder":
            shared_xlim = shared_ylim = None
        else:
            raise ValueError(
                f"Unknown SPATIAL_AXIS_MODE={SPATIAL_AXIS_MODE!r}. "
                "Use 'per_folder', 'per_position', or 'global'."
            )

        # 座標セルの見出し。小図の少し上へ置く。
        label_y = center_y + 0.5 * cell_height_fig + 0.008
        fig.text(
            center_x,
            label_y,
        ...
        for index, result in enumerate(position_results):
            row = index // ncols
            col = index % ncols

            # 図では上の行から詰める。
            left = cell_left + col * (sub_width + gap_x)
            bottom = cell_bottom + (nrows - 1 - row) * (sub_height + gap_y)
            ax = fig.add_axes([left, bottom, sub_width, sub_height], zorder=3)

            if SPATIAL_AXIS_MODE == "per_folder":
                xlim, ylim = robust_limits([result])
            else:
                assert shared_xlim is not None and shared_ylim is not None
            ...
            plot_folder_track(
                ...
                xlim=xlim,
                ylim=ylim,
                title_text=spatial_run_title(result.folder_name, index, n_runs),
                compact=True,
            )

    fig.suptitle(
        "Final corrected IQ tracks arranged by measurement position",
        fontsize=20,
        y=0.975,
        fontweight="bold",
    )
    fig.text(
        0.5,
        0.942,
        "Each mini-panel: horizontal = I_final, vertical = Q_final  |  "
        "thin gray = sampled events  |  plasma = median track  |  black X = pedestal  |  "
        f"IQ axis mode = {SPATIAL_AXIS_MODE}",
        ha="center",
        va="center",
        fontsize=10.5,
    )

    png_path = output_dir / "corrected_iq_tracks_spatial_map.png"
    pdf_path = output_dir / "corrected_iq_tracks_spatial_map.pdf"
    fig.savefig(png_path, dpi=SPATIAL_DPI, bbox_inches="tight")
...
def main() -> None:
    ...
    summary = {
        ...
        "plot": {
            ...
            "ncols": NCOLS,
            "axis_mode": AXIS_MODE,
            "save_spatial_map": SAVE_SPATIAL_MAP,
            "spatial_figsize": list(SPATIAL_FIGSIZE),
            "spatial_dpi": SPATIAL_DPI,
            "spatial_axis_mode": SPATIAL_AXIS_MODE,
            "spatial_z_increases_upward": SPATIAL_Z_INCREASES_UPWARD,
