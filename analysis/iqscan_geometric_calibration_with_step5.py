...
- calibration_step4_final_scan_amp_phase.png
- calibration_step5_waveform_before_after.png
...
# 最終補正後の waveform IQ 図に、補正後の共振円を重ねる
SHOW_RESONANCE_CIRCLE_ON_FINAL_WAVEFORM = True

# True: 共振円全体が見えるように軸範囲を広げる
# False: waveform の robust 範囲を維持し、範囲内にある円弧だけを表示する
INCLUDE_RESONANCE_CIRCLE_IN_AXIS_LIMITS = True
...
def add_iq_waveform_trajectories(
    ...
    xlabel: str,
    ylabel: str,
    resonance_circle_center: complex | None = None,
    resonance_circle_radius: float | None = None,
    include_circle_in_limits: bool = False,
) -> None:
    ...
    # 補正後の共振円を waveform と同じ IQ 平面に重ねる。
    # circle_fit で得た円に alpha, a, phi の変換を施した中心・半径を渡す。
    circle_is_valid = (
        resonance_circle_center is not None
        and resonance_circle_radius is not None
        and np.isfinite(resonance_circle_center.real)
        and np.isfinite(resonance_circle_center.imag)
        and np.isfinite(resonance_circle_radius)
        and resonance_circle_radius > 0.0
    )
    if circle_is_valid:
        add_circle(
            ax,
            center=resonance_circle_center,
            radius=float(resonance_circle_radius),
            label="corrected resonance circle",
        )
    ...
    if np.any(finite):
        xlim = np.asarray(np.nanpercentile(all_x[finite], [0.2, 99.8]), dtype=float)
        ylim = np.asarray(np.nanpercentile(all_y[finite], [0.2, 99.8]), dtype=float)

        if circle_is_valid and include_circle_in_limits:
            center = resonance_circle_center
            radius = float(resonance_circle_radius)
            xlim[0] = min(xlim[0], center.real - radius)
            xlim[1] = max(xlim[1], center.real + radius)
            ylim[0] = min(ylim[0], center.imag - radius)
            ylim[1] = max(ylim[1], center.imag + radius)

        # 円や waveform が枠に接しないよう、共通スケールで少し余白を入れる。
        span = max(
            float(xlim[1] - xlim[0]),
            float(ylim[1] - ylim[0]),
            np.finfo(float).eps,
    ...
    ax.set_aspect("equal", adjustable="box")
    ax.set_title(
        f"{title}\n{event_idx.size}/{z_wave.shape[0]} events, "
...
def main() -> None:
    ...
    # alpha と phi は剛体回転なので半径は不変。
    # a で割る振幅正規化だけが半径を 1/a 倍する。
    resonance_circle_radius_final = circle_fit.radius / geom.amplitude_a
    ...
    with PdfPages(pdf_path) as pdf:
        ...
        # ---------------------------------------------------------------------
        # Figure 4: final corrected IQ-scan amplitude and phase
        # ---------------------------------------------------------------------
        fig, axes = plt.subplots(2, 1, figsize=(12.5, 9.0), sharex=True)
        fig.suptitle(
            "Step 4: amplitude and phase of the final corrected IQ scan",
            fontsize=16,
        )

        amp_final = np.abs(z_scan_final)
        phase_final = np.unwrap(np.angle(z_scan_final))
        i_rf = circle_fit.resonance_index
        ...
        axes[0].scatter(
            ...
            s=130,
            color="tab:red",
            label="scan point nearest to readout",
            zorder=4,
        )
        axes[0].axvline(
            ...
            linestyle=":",
            linewidth=1.2,
            label="requested readout frequency",
        )
        axes[0].set_ylabel(r"amplitude $|z_{\mathrm{final}}|$")
        axes[0].set_title("Amplitude after tau, alpha, a, and phi corrections")
        ...
        axes[1].set_title("Phase after tau, alpha, a, and phi corrections")
        axes[1].grid(alpha=0.3)
        ...
        save_figure(
            fig,
            OUTPUT_DIR / "calibration_step4_final_scan_amp_phase.png",
            pdf,
        )

        # ---------------------------------------------------------------------
        # Figure 5: waveform before/after
        # ---------------------------------------------------------------------
        fig, axes = plt.subplots(2, 2, figsize=(14.0, 12.0))
        fig.suptitle(
            "Step 5: waveform IQ trajectories before / after calibration",
            fontsize=16,
        )
        ...
        add_iq_waveform_trajectories(
            axes[1, 1],
            z_wave_final,
            title="Final corrected waveform IQ + resonance circle",
            xlabel="I_final",
            ylabel="Q_final",
            resonance_circle_center=(
                geom.center_final
                if SHOW_RESONANCE_CIRCLE_ON_FINAL_WAVEFORM
                else None
            ),
            resonance_circle_radius=(
                resonance_circle_radius_final
                if SHOW_RESONANCE_CIRCLE_ON_FINAL_WAVEFORM
                else None
            ),
            include_circle_in_limits=INCLUDE_RESONANCE_CIRCLE_IN_AXIS_LIMITS,
        )

        fig.tight_layout(rect=(0, 0, 1, 0.95))
        save_figure(fig, OUTPUT_DIR / "calibration_step5_waveform_before_after.png", pdf)
