...
(3) 共振付近の z_tau を円 fit して、中心 c と半径 r を求める。

(4) 共振点 z_res に対して、円中心を挟んだ反対側の点
        P = 2c - z_res
    を定義する。
...
- calibration_step4_amp_phase_after_phi.png
...
def main() -> None:
    ...
    circle_fit = fit_circle_near_resonance(z_scan_tau, CIRCLE_HALF_WIDTH_POINTS)
    ...
    with PdfPages(pdf_path) as pdf:
        ...
        add_frequency_colored_iq(
            ...
            extra_points=[
                ...
                (circle_fit.resonance_point, "o", "resonance point", 70),
        ...
        axes[1].axvline(frequency_hz[circle_fit.resonance_index] / 1e9, color="tab:red", linestyle=":", label="resonance")
        ...
        # ---------------------------------------------------------------------
        # Figure 4: amplitude and phase after phi correction
        # ---------------------------------------------------------------------
        fig, axes = plt.subplots(2, 1, figsize=(11.0, 8.0), sharex=True)
        fig.suptitle(
            "IQ scan after full geometric calibration",
            fontsize=16,
        )

        amplitude_final = np.abs(z_scan_final)
        phase_final = np.unwrap(np.angle(z_scan_final))
        resonance_frequency_ghz = (
            frequency_hz[circle_fit.resonance_index] / 1e9
        )
        ...
        axes[0].axvline(
            ...
            label=(
                "resonance candidate "
                f"{resonance_frequency_ghz:.9f} GHz"
            ),
        )
        axes[0].set_ylabel("amplitude")
        axes[0].set_title("Amplitude after phi correction")
        ...
        axes[1].plot(
            ...
            "o-",
            ms=4,
            label=r"$\arg(S_{21,\mathrm{final}})$",
        )
        axes[1].axvline(
            resonance_frequency_ghz,
            color="tab:red",
        ...
        axes[1].set_title("Phase after phi correction")
        axes[1].grid(alpha=0.3)
        ...
        save_figure(
            fig,
            OUTPUT_DIR / "calibration_step4_amp_phase_after_phi.png",
            pdf,
        )
    ...
    params = {
        ...
        "circle_fit": {
            ...
            "resonance_frequency_hz": float(frequency_hz[circle_fit.resonance_index]),
