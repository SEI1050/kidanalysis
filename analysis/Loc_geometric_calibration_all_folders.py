...
def robust_limits(all_results: list[FolderResult]) -> tuple[tuple[float, float], tuple[float, float]]:
    xs = []
    ys = []

    for result in all_results:
        z = result.z_final
        event_idx = choose_event_indices(z.shape[0], min(MAX_EVENTS_TO_PLOT, 40))
        sample_idx = np.arange(0, z.shape[1], max(1, PLOT_SAMPLE_STRIDE * 4))
        sampled = z[np.ix_(event_idx, sample_idx)]

        xs.append(np.real(sampled).ravel())
        ys.append(np.imag(sampled).ravel())

        xs.append(np.real(result.median_track[::max(1, PLOT_SAMPLE_STRIDE)]))
        ys.append(np.imag(result.median_track[::max(1, PLOT_SAMPLE_STRIDE)]))

        xs.append(np.array([result.pedestal.real]))
        ys.append(np.array([result.pedestal.imag]))
    ...
    xpad = 0.05 * (xhi - xlo if xhi > xlo else 1.0)
...
def main() -> None:
    ...
    # 共通軸レンジ
    xlim, ylim = robust_limits(results)
    ...
    summary = {
        ...
        "common_xlim": [xlim[0], xlim[1]],
        "common_ylim": [ylim[0], ylim[1]],
    }
    ...
    with PdfPages(pdf_path) as pdf:
        for page in range(n_pages):
            ...
            for ax, result in zip(axes, page_results):
                plot_folder_track(ax, result, xlim=xlim, ylim=ylim)
