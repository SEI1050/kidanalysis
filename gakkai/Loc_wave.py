...
def load_rf_result(filename):
	...
	event_proj = dev0 * direction[0] + dev1 * direction[1]
	return {
...
def process_position_waveforms(root):
	...
	for (z_position, x_position), timestamps in RF_POSITION_FILES.items():
		timestamp = timestamps[SELECTED_RF_INDEX]
		matches = sorted((root / f"data_0825_{timestamp}").glob("wf_*.npz"))
		if not matches:
			print(f"[skip] missing data_0825_{timestamp} (-2 dBm)")
			continue
		result = load_rf_result(matches[0])
		event_count = result["event_proj"].shape[0]
		selected = rng.choice(
			event_count,
			size=min(RANDOM_EVENT_COUNT, event_count),
			replace=False,
		)
	...
	with PdfPages(output) as pdf:
			...
			for row_index, z_position in enumerate(requested_z):
				for column_index, x_position in enumerate(requested_x):
					...
					if result is not None:
						wave = result[wave_name]
						for event_index in result["selected_events"]:
							event_wave = result[event_name][event_index]
							event_wave = normalize_wave(event_wave)
							axis.plot(
								result["time_ns"] / 1000,
								event_wave,
								color="0.45", alpha=0.08, linewidth=0.5,
							)
						mean_wave = normalize_wave(wave)
						axis.plot(
								result["time_ns"] / 1000,
								mean_wave,
								color="C0", linewidth=1.8,
								label="mean" if row_index == 0 and column_index == 0 else None,
							)
					axis.set_title(f"x={x_position:.1f}, z={z_position:.1f} mm")
					axis.set_xlim(-0.35, 1.65)
					axis.grid(alpha=0.3)
		...
		# Combine every measured position into one correlation plot.  This is
		# useful for comparing an alpha waveform with the full laser response.
		combined_parameters = [
			result["parameters"] for result in position_results.values()
		]
		if combined_parameters:
			parameters = pd.concat(combined_parameters, ignore_index=True)
			amplitude = pd.to_numeric(parameters["amp"], errors="coerce")
			for tau_name, tau_label, tau_threshold in (
				("tau_r_eff", "tau_r_eff", 15.0),
				("tau_d_eff", "tau_d_eff", 50.0),
			):
				tau = pd.to_numeric(parameters[tau_name], errors="coerce")
				valid = (
					np.isfinite(amplitude)
					& np.isfinite(tau)
					& ~((amplitude < 0.01) & (tau < tau_threshold))
				)
				if not valid.any():
					continue
				figure, axis = plt.subplots(figsize=(8, 6))
				histogram = axis.hist2d(
					tau[valid], amplitude[valid], bins=50, cmap="viridis",
				)
				axis.set_title(f"-2 dBm: all positions, amplitude vs {tau_label}")
				...
				pdf.savefig(figure)
				plt.close(figure)
				print(f"combined {tau_label} points = {valid.sum()}")

		# Reproduce ex_ampvstau.py's amplitude/time-constant correlations
		# for each measured x-z position.
		for x_axis_name, x_label, valid_name, threshold in (
			("tau_r_eff", "tau_r_eff", "valid_for_rise", 15.0),
			("tau_d_eff", "tau_d_eff", "valid_for_decay", 50.0),
		):
			figure, axes = plt.subplots(
				len(requested_z), len(requested_x), figsize=(12, 15),
				squeeze=False, sharex=True, sharey=True,
			...
			for row_index, z_position in enumerate(requested_z):
				for column_index, x_position in enumerate(requested_x):
					...
					if result is not None:
						...
						amplitude = pd.to_numeric(parameters["amp"], errors="coerce")
						tau = pd.to_numeric(parameters[x_axis_name], errors="coerce")
						valid = (
							np.isfinite(amplitude)
							& np.isfinite(tau)
							& ~((amplitude < 0.01) & (tau < threshold))
						)
						if valid.any():
							histogram = axis.hist2d(
								tau[valid], amplitude[valid], bins=50, cmap="viridis",
				...
				figure.suptitle(f"-2 dBm: amplitude vs {x_label}")
				figure.tight_layout()
				pdf.savefig(figure)
				plt.close(figure)
	print("saved:", output)
...
def main():
	...
	if os.path.isdir(input_path):
		...
		process_position_waveforms(Path(input_path))
	else:
